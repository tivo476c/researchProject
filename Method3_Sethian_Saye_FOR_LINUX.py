...
def all_my_midpoints(N_Cell):
    ...
    for i in range(N_Cell):
        ...
        frame_time = pos_phase[pos_phase["time"]==Active_time_step]
...
def startRunMethod3(midpointsDirectoryPath, phasefieldDirectoryPath, output_path, time):
    ...
    global Dirty_dir_time_path
    Dirty_dir_time_path = os.path.join(output_path,  f"dirty_vertices_time_{time}")
    if not os.path.exists(Dirty_dir_time_path):
